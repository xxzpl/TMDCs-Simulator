...
!~ ==========================================================
subroutine Hamiltonianofxs2VII_DATA ( neigenvalues,  XK, Yk, tdp, tdd, tpp,Delta0,Delta1,Delta2,DeltaP,DeltaZ, Vonset, eig, vec) 
use moduleone
...
!~ end do
	...
	open (unit =UNIT4, file = FILENAME4,  status="UNKNOWN" , ACTION="WRITE", iostat=ierror)  
	open (unit =UNIT17, file = FILENAME17,  status="UNKNOWN" , ACTION="WRITE", iostat=ierror)  
	
	do i=1, neigenvalues
		write(UNIT4,  '(  22(f8.3) )'  )  ( real(Hamiltonian( i, j )),  j=1, neigenvalues)
		!~ write(UNIT4, '( a )' ) 
		write(UNIT17,  '(  22(f8.3) )'  )  ( aimag(Hamiltonian( i, j )),  j=1, neigenvalues)
		!~ write(UNIT17, '( a )' )
	end do
	close(UNIT4)
    close(UNIT17)
...
end subroutine Hamiltonianofxs2VII_DATA
